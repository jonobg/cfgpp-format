...
from .schema_parser import (
    SchemaDocument, SchemaDefinition, SchemaField, EnumDefinition,
    ValidationRule, FieldRequirement, SchemaParseError
)
...
class SchemaValidator:
    ...
    def validate(self, config_data: Dict[str, Any], schema_name: Optional[str] = None) -> ValidationResult:
        """
        ...
        try:
            ...
            if schema_name not in self.schema_doc.schemas:
                self._add_error(f"Schema '{schema_name}' not found in schema document", "")
            ...
            is_valid = not any(msg.severity == ValidationSeverity.ERROR for msg in self.messages)
    ...
    def _validate_object(self, obj_data: Dict[str, Any], schema: SchemaDefinition, path: str):
        """
    ...
    def _validate_type(self, value: Any, type_name: str, path: str):
        ...
        if type_name == 'string':
            if not isinstance(actual_value, str):
                self._add_error(f"Expected string but got {type(actual_value).__name__}", path)
        elif type_name == 'int':
            if not isinstance(actual_value, int):
                self._add_error(f"Expected int but got {type(actual_value).__name__}", path)
        elif type_name == 'float':
            if not isinstance(actual_value, (int, float)):
                self._add_error(f"Expected float but got {type(actual_value).__name__}", path)
        elif type_name == 'bool':
            if not isinstance(actual_value, bool):
                self._add_error(f"Expected bool but got {type(actual_value).__name__}", path)
            ...
            if isinstance(value, dict) and 'body' in value:
                ...
                self._add_error(f"Expected object of type '{type_name}' but got {type(actual_value).__name__}", path)
        else:
            # Unknown type - might be a custom type not yet defined
            self._add_warning(f"Unknown type '{type_name}' - cannot validate", path)
    
    def _validate_schema_rules(self, obj_data: Dict[str, Any], schema: SchemaDefinition, path: str):
        """
        ...
        for rule in schema.validation_rules:
            try:
                ...
                self._add_warning(f"Could not evaluate validation rule: {rule.expression}", path)
    
    def _validate_simple_rule(self, expression: str, obj_data: Dict[str, Any], path: str) -> bool:
        """
...
def validate_config(config_data: Dict[str, Any], schema_doc: SchemaDocument, 
