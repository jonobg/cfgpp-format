...
class CfgppFormatter:
    ...
    def __init__(self, config: Optional[FormatterConfig] = None, schema_doc: Optional[SchemaDocument] = None):
        """
    ...
    def format(self, config_text: str) -> str:
        ...
        try:
            ...
            raise ConfigParseError(f"Cannot format invalid configuration: {e}", e.line, e.col)
    ...
    def _format_array_value(self, array_items: List[Dict[str, Any]]) -> str:
        ...
        if self.config.array_style == ArrayStyle.COMPACT:
            ...
            items_str = ',\n'.join(f"{self._get_indent()}{item}" for item in formatted_items)
            return f"[\n{items_str}\n{self._get_indent()[:-4]}]"
        else:  # AUTO
            total_length = sum(len(item) for item in formatted_items) + len(formatted_items) * 2
    ...
    def _format_array(self, arr: List[Any], inline_context: bool = False) -> str:
        ...
        if self.config.array_style == ArrayStyle.ONE_PER_LINE:
            ...
            total_length = sum(len(item) for item in formatted_items) + len(formatted_items) * 2  # commas and spaces
            should_wrap = (
                total_length > self.config.array_wrap_threshold or
                len(formatted_items) > self.config.array_element_threshold or
    ...
    def _is_type_inferred(self, prop: Dict[str, Any]) -> bool:
        ...
        if isinstance(value, dict) and 'type' in value:
            ...
            return True
        elif isinstance(value, (int, float)) and prop_type in ['int', 'float', 'number']:
            return True
...
def format_string(config_text: str, config: Optional[FormatterConfig] = None, 
...
def format_file(input_path: str, output_path: Optional[str] = None, 
               config: Optional[FormatterConfig] = None,
               schema_path: Optional[str] = None) -> str:
    """
    Format a cfgpp file.
