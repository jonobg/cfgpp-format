...
def add_formatter_commands(subparsers):
    ...
    format_parser.add_argument("files", nargs="+", help="Configuration files to format")
    ...
    format_parser.add_argument(
        "--output", "-o", help="Output directory (when not using --in-place)"
    )
    format_parser.add_argument(
        "--config", "-c", help="Path to formatting configuration file (.cfgpp-format)"
    )
    format_parser.add_argument(
        "--style",
    ...
    format_parser.add_argument(
        "--diff", action="store_true", help="Show diff of changes (implies --check)"
    )
    format_parser.add_argument("--indent", type=int, help="Override indentation size")
    format_parser.add_argument(
        "--tabs", action="store_true", help="Use tabs instead of spaces"
    )
    format_parser.add_argument(
        "--max-line-length", type=int, help="Maximum line length"
    )
    format_parser.add_argument(
        "--brace-style",
    ...
    check_parser.add_argument("files", nargs="+", help="Configuration files to check")
    check_parser.add_argument(
        "--config", "-c", help="Path to formatting configuration file"
    )
    check_parser.add_argument(
        "--style",
        choices=["default", "compact", "expanded"],
...
def cmd_format_init(args) -> int:
    ...
    try:
        ...
        if output_path.exists() and not args.force:
            print(
                f"❌ Configuration file already exists: {output_path}", file=sys.stderr
            )
...
def _load_formatter_config(args) -> FormatterConfig:
    ...
    if config_file and config_file.exists():
        try:
            ...
            print(
                f"⚠️  Warning: Could not load config file {config_file}: {e}",
                file=sys.stderr,
            )
