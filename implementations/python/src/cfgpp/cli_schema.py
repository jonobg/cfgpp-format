...
from .schema_integration import (
    load_with_auto_schema, loads_with_schema, auto_discover_schema,
    format_validation_messages, SchemaRegistry, register_global_schema
)
...
def add_schema_commands(subparsers):
    ...
    validate_parser.add_argument(
        '--schema-name',
        help='Specific schema name within the schema document'
    )
    validate_parser.add_argument(
        '--strict',
        action='store_true',
    ...
    check_parser.add_argument(
        'schema_file',
        help='Path to the schema file to check'
    )
    check_parser.add_argument(
        '--format',
        choices=['human', 'json'],
    ...
    info_parser.add_argument(
        '--schema-name',
        help='Show info for specific schema only'
    )
    info_parser.add_argument(
        '--format',
        choices=['human', 'json'],
    ...
    try:
        ...
        if args.schema:
            ...
            if discovered_schema:
                ...
                print("⚠️  No schema file discovered. Use --schema to specify one.", file=sys.stderr)
                return 2
...
def cmd_schema_check(args) -> int:
    ...
    try:
        ...
        if args.format == 'json':
            ...
            print(f"📊 Found {len(schema_doc.schemas)} schema(s): {', '.join(schema_doc.schemas.keys())}")
            print(f"🏷️  Found {len(schema_doc.enums)} enum(s): {', '.join(schema_doc.enums.keys())}")
...
def output_human_validation(validation_result, quiet: bool = False, strict: bool = False):
...
def output_schema_info_human(schema_doc, specific_schema: Optional[str] = None):
    ...
    if schema_doc.schemas:
        ...
        for schema_name, schema_def in schemas_to_show.items():
            ...
            # Required fields
            required_fields = [f for f in schema_def.fields.values() 
            ...
            # Optional fields
            optional_fields = [f for f in schema_def.fields.values() 
                             if f.requirement.value == 'optional']
            if optional_fields:
                print(f"      Optional fields ({len(optional_fields)}):")
                for field in optional_fields:
                    array_suffix = "[]" if field.is_array else ""
                    default_info = f" = {field.default_value}" if field.default_value is not None else ""
                    print(f"        • {field.type_name}{array_suffix} {field.name}{default_info}")
...
def output_schema_info_json(schema_doc, specific_schema: Optional[str] = None):
    ...
    result = {
        'enums': {
            name: {
                ...
                'default': enum_def.default_value
            }
    ...
    for schema_name, schema_def in schemas_to_include.items():
        result['schemas'][schema_name] = {
            ...
            'validation_rules': [rule.expression for rule in schema_def.validation_rules]
        }
