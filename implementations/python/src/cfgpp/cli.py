...
def format_cfgpp(data: Dict[Any, Any], indent: int = 2) -> str:
    ...
    def format_array(arr, current_indent=0):
        ...
        if len(formatted_items) <= 3 and all(
            len(item) < 20 for item in formatted_items
        ):
...
Examples:
  ...
  cfgpp schema-info schema.cfgpp-schema
    ...
    parser.add_argument(
        "-", dest="stdin", action="store_true", help="Read from standard input"
    )
    ...
    try:
        ...
        if hasattr(e, "line") and hasattr(e, "column"):
            print(
                f"Parse error at line {e.line}, column {e.column}: {e.message}",
                file=sys.stderr,
            )
        else:
