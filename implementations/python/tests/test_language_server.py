...
from cfgpp.tools.language_server import (
    CfgppLanguageServer,
    DocumentManager,
    DiagnosticsEngine,
    CompletionProvider,
...
class TestCfgppLanguageServer(unittest.TestCase):
    ...
    def test_diagnostic_conversion(self):
        ...
        diagnostic = Diagnostic(
            range=Range(
                start=Position(line=1, character=5), end=Position(line=1, character=15)
            ),
            severity=1,
            code="test_error",
            source="cfgpp",
...
class TestLSPIntegration(unittest.TestCase):
    ...
    def test_document_lifecycle(self):
        ...
        self.assertEqual(
            self.server.document_manager.get_document_content(uri), content
        )
        ...
        self.assertEqual(
            self.server.document_manager.get_document_content(uri), new_content
        )
