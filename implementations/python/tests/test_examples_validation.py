...
class TestExamplesValidation:
    ...
    def setup_class(cls):
        ...
        assert (
            len(cls.example_files) > 0
        ), f"No .cfgpp files found in {cls.examples_dir}"
    ...
    def test_all_examples_parse_successfully(self):
        ...
        for example_file in self.example_files:
            try:
                ...
                assert isinstance(
                    result, dict
                ), f"Parser should return dict, got {type(result)} for {example_file}"

                print(
                    f"✅ {example_file.relative_to(self.examples_dir)} - {parsing_time:.4f}s"
                )

            except (ConfigParseError, LexerError) as e:
                failed_files.append((example_file, str(e)))
                print(
                    f"❌ {example_file.relative_to(self.examples_dir)} - Parse Error: {e}"
                )

            except Exception as e:
                failed_files.append((example_file, f"Unexpected error: {str(e)}"))
                print(
                    f"💥 {example_file.relative_to(self.examples_dir)} - Unexpected Error: {e}"
                )
    ...
    def test_basic_examples_structure(self):
        ...
        for expected_file in expected_basic_files:
            ...
            assert (
                file_path.exists()
            ), f"Expected basic example {expected_file} should exist"
            ...
            assert isinstance(
                result, dict
            ), f"Basic example {expected_file} should parse to dict"
    ...
    def test_ai_aware_examples_structure(self):
        ...
        for ai_file in ai_aware_files:
            ...
            if "hash" in content and (
                "ConfigMetadata" in content or "SectionHashes" in content
            ):
            ...
            assert (
                len(ai_features_found) > 0
            ), f"AI-aware example {ai_file.name} should contain AI feature annotations"

            # Parse the configuration (ignoring AI metadata for now)
            result = loads(content)
            assert isinstance(
                result, dict
            ), f"AI-aware example {ai_file.name} should parse to dict"
    ...
    def test_example_directories_have_readmes(self):
        ...
        for subdir in example_subdirs:
            ...
            assert (
                readme_path.exists()
            ), f"Directory {subdir.name}/ should have a README.md file"

            # README should not be empty
            with open(readme_path, "r", encoding="utf-8") as f:
                readme_content = f.read().strip()
            assert (
                len(readme_content) > 0
            ), f"README.md in {subdir.name}/ should not be empty"
    ...
    def test_performance_benchmarks(self):
            ...
            performance_results.append(
                {
                    ...
                    "size": file_size,
                    "throughput": (
                        file_size / avg_time if avg_time > 0 else float("inf")
                    ),
                }
            )
        ...
        for result in performance_results:
            throughput_str = (
                f"{result['throughput']:.0f} B/s"
                if result["throughput"] != float("inf")
                else "∞"
            )
            print(
                f"{str(result['file']):<40} {result['size']:<8} {result['avg_time']:.4f}s   {throughput_str:<12}"
            )

        # Performance assertions
        max_acceptable_time = 0.1  # 100ms should be plenty for any example
        slow_files = [
            r for r in performance_results if r["avg_time"] > max_acceptable_time
        ]

        if slow_files:
            slow_files_str = ", ".join(str(f["file"]) for f in slow_files)
            pytest.fail(
                f"Example files taking too long to parse (>{max_acceptable_time}s): {slow_files_str}"
            )
    ...
    def test_example_content_quality(self):
        ...
        for example_file in self.example_files:
            ...
            ), f"Example {example_file.name} should have explanatory comments"
            ...
            assert (
                len(non_comment_lines) >= 3
            ), f"Example {example_file.name} should have substantial content (at least 3 non-comment lines)"
